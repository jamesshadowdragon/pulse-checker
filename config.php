...
{
    return $_SERVER['HTTP_X_PULSECHECK_TOKEN'] ?? ($_GET['token'] ?? '');
}
...
function writeMonitor(array $monitor): bool
{
    ...
    $path = monitorPath($id);
    $tmp = $path . '.tmp';
    $json = json_encode($monitor, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES);
    if ($json === false) {
        return false;
    }
...
function nextMonitorId(): int
{
    ...
    foreach ($files as $file) {
        if (preg_match('/monitor_(\d+)\.json$/', $file, $m)) {
            $max = max($max, (int) $m[1]);
        }
    }
    return $max + 1;
